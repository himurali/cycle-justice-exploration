...
import documentariesData from "@/constants/documentaries.json";

// Types for our documentary data
type FilmTag = string;
...
// Parse the JSON data to ensure it matches our Documentary type
const documentaries: Documentary[] = documentariesData as Documentary[];
...
const Documentaries = () => {
  ...
  // Filter documentaries by selected continent
  const filteredDocumentaries = selectedContinent === "All Continents" 
    ? documentaries 
    : documentaries.filter(doc => doc.continent === selectedContinent);
  
  // Calculate pagination
  const totalItems = filteredDocumentaries.length;
  ...
  const currentItems = filteredDocumentaries.slice(startIndex, endIndex);
  ...
  const handleShareClick = (event: React.MouseEvent, doc: Documentary) => {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(doc.videoUrl)
        .then(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-20">
        {/* Header Section - below the NavBar with padding top to avoid overlap */}
        <div className="bg-gradient-to-b from-gray-800 to-gray-600 py-16 px-4 rounded-b-3xl mb-8">
          <div className="container mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-2 font-baskerville">Documentaries</h1>
            <p className="text-lg text-gray-200 max-w-2xl mx-auto">
              Deep dives into cycling issues, urban planning, and transportation equity
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {currentItems.map((doc) => (
              <Card 
                ...
                onClick={() => handleVideoClick(doc.videoUrl)}
              >
